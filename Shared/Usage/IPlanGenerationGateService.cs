...
/// DB, nunca el claim del JWT), duración pedida y cupo de planes guardados, y por último
/// consume el contador de generación (3/mes free · cap antiabuso 50/día Plus).
/// </summary>
public interface IPlanGenerationGateService
{
    /// <summary>
    /// Ejecuta los checks en orden (tier → duración → planes guardados → contador) y
    /// consume 1 permiso del contador SOLO si todos pasan. Un rechazo de cualquier gate
    /// previo NO consume contador; una vez consumido, la generación "ha arrancado" y el
    /// permiso no se devuelve aunque el LLM falle después (semántica elegida, ver README
    /// de Billing).
    /// </summary>
