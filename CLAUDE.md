...
│   │   ├── AccountDevController.cs     # POST /account/dev/tier + /account/dev/reset-quota (INTERNAL; ambos comparten gates fail-closed vía TryGateAsync, DOBLE fail-closed y OPACO 404, YA NO por entorno: (1) Dev:TierOverrideEnabled + (2) email de la fila de DB ∈ Dev:AllowedEmails con match EXACTO case-insensitive+trim (NO por dominio); allowlist vacío O flag off → 404; efecto SIEMPRE sobre el User.Id del token. Funciona contra Railway PROD abriéndolo explícitamente. Deploy: Dev__TierOverrideEnabled=true + Dev__AllowedEmails__0=<email exacto>)
│   │   └── DevOptions.cs               # Config "Dev" (TierOverrideEnabled default false + AllowedEmails default vacío; ambos deben abrirse a mano en Railway para habilitar los endpoints dev)
│   ├── Admin/
│   │   ├── Analytics/
