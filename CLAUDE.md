...
- **T1 — endpoint `POST /import/video`** (`ImportController`): feature del catálogo **Plus**. `[Authorize]` AppScheme (anónimo → 401) + gate Plus (tier FRESCO de DB → free = `403 import_requires_plus`). v1 = contenido PROPIO (`platform=self`); `platform≠self` con `Import:ThirdPartyEnabled` off → `403 third_party_import_disabled`. Multipart con streaming a temp file (borrado en `finally`); cap 150 MB SOLO aquí vía `[RequestSizeLimit]` (`[DisableFormValueModelBinding]` para que MVC no consuma el body antes que el `MultipartReader`). Rechazo barato ANTES de cuota: MIME → `400 import_unsupported_format`, tamaño → `400 import_too_large`. Cuota por usuario sobre `usage_counters` (**30/mes + 10/día**, ventanas `import_monthly`/`import_daily`, TOCTOU-safe) → agotada = `429 import_limit_reached` (`window`; 429 y no 403 porque ya es Plus, sin upsell — como el cap diario Plus). Reembolso (`IUsageCounterService.ReleaseAsync`, decrement atómico con suelo): éxito y `no_places_found` (Gemini pagó) mantienen cuota; `ExtractionUnavailable`/`VideoTooLong`/`VideoTooLarge` reembolsan ambas ventanas. Respuestas: 200 (candidatos saneados, ciudad/idioma/vibes/confianza + `platform`/`creatorHandle`; SIN internals de Gemini), `422 no_places_found`, `503 import_unavailable`. NO persiste plan todavía (matching + creación = T3/T4). Capability para la app en `GET /account` (`importThirdPartyEnabled`).
...
│   │   ├── ImportDtos.cs               # ImportVideoResponse/ImportPlaceDto (proyección recortada, sin internals de Gemini)
...
| Import | `POST /import/video` (F2 T1; `[Authorize]` AppScheme + gate Plus; multipart, 150 MB solo aquí; gating terceros `Import:ThirdPartyEnabled`; cuota 30/mes·10/día → `429 import_limit_reached`; `ImportLimit` 20/hr por IP) |
