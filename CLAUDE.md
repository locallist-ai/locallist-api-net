...
│   │   ├── AccountDevController.cs     # POST /account/dev/tier + /account/dev/reset-quota (INTERNAL; ambos comparten gates fail-closed vía TryGateAsync, DOBLE fail-closed y OPACO 404, YA NO por entorno: (1) Dev:TierOverrideEnabled + (2) email de la fila de DB ∈ Dev:AllowedEmails con match EXACTO Ordinal case-sensitive SIN trim (espeja la unicidad case/whitespace-sensitive de users.email → variante de caja/espacios NO matchea, cierra el self-upgrade; NO por dominio); allowlist vacío O flag off → 404; efecto SIEMPRE sobre el User.Id del token. Funciona contra Railway PROD abriéndolo explícitamente. Deploy: Dev__TierOverrideEnabled=true + Dev__AllowedEmails__0=<email EXACTO, misma caja con la que la cuenta interna está registrada>)
