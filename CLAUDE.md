...
- **T3 — matching contra catálogo** (`ImportMatchingService`): tras la extracción, el endpoint matchea cada candidato contra los places **published** de la ciudad detectada (normalizada con `CityNameNormalizer`) y enriquece el DTO con `matchedPlaceId` (Guid?), `matchedPlaceName` y `matchConfidence` (`high`|`medium`|null) — ADITIVO, no rompe el contrato de T1. Los no matcheados salen igual (la app los pinta como "no está en LocalList"). **NO** crea places (curación = admin) ni llama a Google (coste/ToS). Matching DETERMINISTA (v1, sin trgm de DB): UNA query trae los places de la ciudad (proyección id/name/city) y matchea en memoria (catálogos por ciudad ~100s; import Plus-only + rate-limited). Estrategia: normaliza nombres (lower + sin diacríticos + colapsa no-alfanumérico; ignora ruido genérico —artículos + "restaurant/bar/cafe/coffee"— SOLO para comparar). `high` = igualdad normalizada exacta **o** un run contiguo de tokens con el lado corto ≥ 5 chars. `medium` = solape ≥ 60% de los tokens del candidato **y** ≥ 2 tokens en común (un único token genérico NUNCA matchea). Empates deterministas: mayor confianza → mayor solape → nombre más ajustado → más corto → menor Id (orden total). Ciudad detectada ausente o no presente en catálogo → todos unmatched, SIN error. `VideoExtractionResult.MetricId` propaga la fila de diagnóstico para que el endpoint anote `video_import_metrics.num_matched` (un UPDATE por PK, best-effort). El extractor v1 no aporta geo, así que el match es solo por nombre.
...
│   │   ├── ImportDtos.cs               # ImportVideoResponse/ImportPlaceDto (proyección recortada + campos de match T3, sin internals de Gemini)
