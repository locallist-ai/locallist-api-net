...
| **Rate Limit** | 100 req/min global. Builder/chat-generate (sliding window, encadenado): techo 60/hr por IP anti account-farming (`Builder__RateLimitPerHourPerIp`) + refinamiento por identidad 5/hr anon por IP · 20/hr auth por userId SOLO AppScheme (`Builder__RateLimitPerHour` / `Builder__RateLimitPerHourAuthenticated`). Chat 20/hr anon · 40/hr auth. Auth 10/15min. Waitlist 5/60s. Admin 60/min. `UseRateLimiter` va después de `UseAuthentication`. |
