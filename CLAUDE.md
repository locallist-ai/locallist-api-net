...
| Import | `POST /import/video?platform=&creatorHandle=` (F2 T1; `[Authorize]` AppScheme + gate Plus; metadatos por query = gating terceros pre-body, multipart solo-fichero 150 MB solo aquí; cuota 30/mes·10/día → `429 import_limit_reached`, reembolso solo si Gemini NO facturó; `ImportLimit` 20/hr por IP) |
