...
- Con config activa, la ingesta (create/bulk/import-from-urls) rehostea cada foto server-side (descarga validada → webp máx 1200px vía ImageSharp → upload S3, cliente S3 con `Timeout=10s`/`MaxErrorRetry=1`) y `Place.Photos` solo persiste URLs de R2; una URL de Google con `key=` que falle el rehost se descarta (nunca se persiste). Un fallo/cuelgue de R2 degrada a "sin foto" — jamás aborta la creación del place. Import masivo commitea por chunks de 10 (una cancelación no pierde lo completado).
- `POST /admin/places/backfill-photos` migra lo existente (excluye `deleted`/`rejected`) y devuelve censo por dominio. Un solo barrido a la vez (409 `backfill_already_running`); fallos por-fuente entran en backoff (`deferredPlaces`, reintento con `retryDeferred=true`) para que el bucle converja a `remainingPlaces=0`; 3 fallos seguidos de upload a R2 abortan el barrido (`aborted`/`abortReason`) para no facturar Google sin progreso; `recoverMissing=true` recupera places sin fotos vía `GooglePlaceId` (Places Details).
...
- Concurrencia: `Place` usa `xmin` de Postgres como concurrency token (migración no-op `UseXminConcurrencyOnPlaces`). El backfill hace reload+merge ante conflicto; `PATCH /admin/places/:id` (y review/postpone) devuelven 409 `concurrent_update` en escritura stale — nunca lost update silencioso de `Photos`.
