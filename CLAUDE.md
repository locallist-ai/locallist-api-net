...
- `Import__MaxDurationSeconds` (600), `Import__MaxSizeBytes` (157286400 = 150MB, VÍDEO), `Import__AllowedMimeTypes` (mp4/quicktime/webm), `Import__FilePollDelayMs` (1000), `Import__FilePollMaxAttempts` (60).
- **Import acepta VÍDEO e IMAGEN** (la File API de Gemini procesa imágenes nativamente y SIN coste por duración — más barato que el vídeo). `Import__AllowedImageMimeTypes` (jpeg/png/webp/heic) es una **lista SEPARADA** de la de vídeo a propósito: aguas abajo hay que distinguir el tipo de media para SALTAR el check de duración (paso 3 de `VideoExtractionService`) y aplicar caps distintos. `Import__MaxImageSizeBytes` (26214400 = 25MB) cap propio de imagen (una captura/lista/carrusel es pequeña; no necesita el ancho del vídeo). El MIME decide el camino: `ImportOptions.IsImageMime/IsVideoMime/IsAllowedMime/SizeCapFor`. Un MIME fuera de ambas listas (p.ej. `application/pdf`) → `400 import_unsupported_format` pre-subida.
...
- **Privacidad del DTO anónimo**: `GET /plans/shared/:token` sirve `PlanDetailDto` con los campos del DUEÑO excluidos (`with { CreatedById = null, TripContext = null }` + global WhenWritingNull → ausentes del JSON): `createdById` (identificador interno) y `tripContext` (dieta, presupuesto, groupType, exclusiones — preferencias personales del owner). `startDate` SE QUEDA (decisión hub 2026-07-27: es contenido útil del itinerario para el receptor, no dato del dueño). El resto (nombre, ciudad, stops, atribución de import) ya es contenido del plan.
- **Clonar un plan (`POST /plans/:id/clone`)**: "guardar este plan como mío" — gancho de conversión del onboarding (el showcase se muestra sin cuenta; guardarlo exige registro, endpoint `[Authorize]` AppScheme → anónimo 401). Origen CLONABLE = `IsShowcase==true` (curado admin, clonable incondicional) O `Visibility=='public'` (plan de usuario público, honra bloqueos vía `IPlanAccessService.CanView`); privado/unlisted ajeno o inexistente → 404 opaco. Crea copia PROFUNDA propiedad del caller: nuevo Guid, TODOS los `PlanStop` con nuevos ids (place_id/orden/día/horario copiados fielmente), `Name`/`Description`/`City`/`Type`/`DurationDays`/`ImageUrl` copiados, `CreatedById=caller`, `Visibility='private'`, `Source="cloned"`, `IsShowcase=false`, `ClonedFrom={origen}`; i18n (`NameI18n`/`DescriptionI18n`) copiado tal cual. **`source="cloned"` = plan de usuario NORMAL**: NUNCA sale en showcase (`GET /plans?showcase=true` filtra por `IsShowcase`) ni se trata como curated (el gate i18n `isCurated` mira `Source=="curated"`, así que el ES del showcase se conserva sin exigir `translation_status` approved). **Decisiones hub 2026-07-27**: `StartDate` NO se hereda (un showcase con fecha fija/pasada daría un clon fechado en el pasado + choca con viabilidad; el usuario pone su fecha al seguir/editar) · `TripContext` se copia SOLO de un showcase curado (contexto de diseño del itinerario); de un plan PÚBLICO ajeno → **null** (dieta/presupuesto/grupo/exclusiones son datos personales de un extraño, no se persisten bajo el cloner). Cuenta como plan guardado → mismo gate del cupo que `POST /plans` (free con `FreeSavedPlansLimit` → 403 `saved_plans_limit_reached`; Plus ilimitado; tier fresco de DB vía `TierGate`). **Idempotencia ATÓMICA**: un usuario tiene A LO SUMO un clon activo por origen, garantizado por el índice único PARCIAL `(created_by, cloned_from) WHERE cloned_from IS NOT NULL` (migración `AddPlanClonedFromUniqueIndex`). El pre-check SELECT es solo la vía rápida del doble-tap NO concurrente; contra N clones concurrentes del mismo origen el que pierde recibe 23505, re-lee el ganador y devuelve ESE (200) — mismo patrón que el índice único de Favorites (DELETE es hard, "activo"=existe). Clon nuevo → 201; existente/ganador → 200. `cloned_from` (S1) es plan→plan del clone; distinto de `imported_from_platform`/`imported_creator_handle` (F2, vídeo→plan).
...
│   │   ├── AccountDevController.cs     # POST /account/dev/tier + /account/dev/reset-quota (INTERNAL; ambos comparten gates fail-closed vía TryGateAsync, DOBLE fail-closed y OPACO 404, YA NO por entorno: (1) Dev:TierOverrideEnabled + (2) email de la fila de DB ∈ Dev:AllowedEmails con match EXACTO Ordinal case-sensitive SIN trim (defensa-en-profundidad del self-upgrade por variante — cubre datos legados sin normalizar; desde la higiene de email users.email es citext+normalizado, así que una variante de caja ni se materializa; NO por dominio); allowlist vacío O flag off → 404; efecto SIEMPRE sobre el User.Id del token. Funciona contra Railway PROD abriéndolo explícitamente. Deploy: Dev__TierOverrideEnabled=true + Dev__AllowedEmails__0=<email EXACTO, misma caja con la que la cuenta interna está registrada>)
│   │   └── DevOptions.cs               # Config "Dev" (TierOverrideEnabled default false + AllowedEmails default vacío, match exacto case-sensitive; ambos deben abrirse a mano en Railway para habilitar los endpoints dev)
│   ├── Admin/
│   │   ├── Analytics/
...
└── Shared/
    ...
    │   ├── EmailNormalizer.cs          # Normalize(email) = lower+trim. Punto ÚNICO de email canónico (register/login/signin SSO/sync/waitlist); pareja de citext en users.email
...
| Feature | Endpoints |
|---|---|
| Account | `GET /account` (+ `aiPlansMonth` cuota mensual y `importThirdPartyEnabled` capability F2), `DELETE /account`. **INTERNAL** (`[Authorize]` AppScheme; ambos comparten gates fail-closed vía `TryGateAsync`, **DOBLE fail-closed y opaco 404, YA NO por entorno**: (1) `Dev:TierOverrideEnabled` default false → 404 · (2) email del usuario (fila de DB, resuelto por el `User.Id` del token) ∈ `Dev:AllowedEmails` con match **EXACTO `Ordinal` case-sensitive, SIN trim** (defensa-en-profundidad del self-upgrade por variante — sigue cubriendo datos legados sin normalizar; desde la higiene de email `users.email` es `citext`+normalizado en escritura, así que una variante `PABLO@…`/` pablo@…` ya NO puede materializarse como otra fila; NO por dominio: el viejo check `@locallist.ai` era inerte porque `/auth/register` no verifica buzón; el exacto es seguro porque `users.email` es único y no hay endpoint de cambio de email sin verificar) → si no 404 · allowlist vacío (default) O flag off → 404; efecto siempre sobre el id del token, nunca por el body): `POST /account/dev/tier` (flipa el `User.Tier` REAL para testear flujos Plus; `tier∈{pro,free}` estricto o 400; no toca `rc_customer_id`/RevenueCat) · `POST /account/dev/reset-quota` (borra las filas de `usage_counters` del caller — planes IA/mes + import mensual/diario → cuotas a 0; `{reset: nº filas}`). Funciona contra Railway PROD (donde viven los datos) abriéndolo a mano: `Dev__TierOverrideEnabled=true` + `Dev__AllowedEmails__0=<email EXACTO, misma caja con la que la cuenta interna está registrada>` |
...
| Plans | `GET /plans/` (listado público: filtra por `visibility=='public'`, un `unlisted` JAMÁS aparece), `GET /plans/mine`, `GET /plans/:id`, `POST /plans` (crea plan de usuario; gate del cupo de guardados free = 5), `POST /plans/:id/clone` (`[Authorize]` AppScheme; "guardar este plan como mío" — gancho de conversión del onboarding), `PUT /plans/:id/stops` (reemplazo atómico de stops, día ≤14), `DELETE /plans/:id` |
| Plans — Social S1 (share-link) | `POST /plans/:id/share` (`[Authorize]` AppScheme, SOLO owner vía `IPlanAccessService.IsOwner`; genera `share_token` cripto-random URL-safe si no existe y sube `private→unlisted`; idempotente = mismo token, no re-baja; 404 opaco si no owner; `{shareToken, visibility}`) · `DELETE /plans/:id/share` (owner; revoca = `share_token=null` y `unlisted→private`; `public` sigue public sin token; idempotente 204) · `GET /plans/shared/:token` (`[AllowAnonymous]` + `SharedPlanLimit` 60/min por IP; resuelve POR TOKEN, nunca por GUID; `PlanDetailDto` solo-lectura si `visibility ∈ {unlisted,public}` y coincide, con `createdById` Y `tripContext` EXCLUIDOS por privacidad — `startDate` se queda; token inválido/revocado/plan-vuelto-a-private → 404 indistinguible; re-share tras revoke = token NUEVO, el enlace revocado no revive) |
...
- **Higiene de email (email normalizado + `citext`)**: `users.email` es **`citext`** (unicidad e igualdad case-insensitive a nivel de esquema; el índice único `IX_users_email` es case-insensitive). Además TODA escritura/comparación de email normaliza (lower+trim) por un punto ÚNICO — `EmailNormalizer.Normalize` (`Shared/Auth/`) — aplicado en register/login (dup-check + `Email=`), signin SSO (link por email de cuentas legadas + creación; el claim del provider llega crudo → se normaliza en el controller), sync (Firebase) y waitlist. Efecto: `Pablo@x.com` / ` pablo@x.com ` / `PABLO@X.COM` son la MISMA cuenta; se acabaron los duplicados por variante. El gate Ordinal exact-match de `/account/dev/*` (`AccountDevController`) sigue vigente como defensa-en-profundidad (cubre datos legados sin normalizar), aunque citext + normalización ya impiden materializar la variante.
- **NOTA DE DEPLOY (migración `NormalizeEmailCitext`)**: la migración lleva un GUARD (`RAISE EXCEPTION` ANTES del UPDATE/ALTER, todo en su transacción) que ABORTA LIMPIO — sin tocar datos — si al normalizar dos filas colisionarían. Antes de aplicarla en Railway prod, correr `SELECT lower(trim(email)) norm, count(*) FROM users GROUP BY 1 HAVING count(*)>1;` y resolver manualmente los duplicados que aparezcan (fusionar/renombrar); si no hay filas, la migración pasa sola. `Down()` revierte SOLO el tipo a `varchar(255)` (los valores ya normalizados se quedan, es inocuo).
