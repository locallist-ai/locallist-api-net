...
- `R2__Bucket` — opcional, default `locallist-images`. `R2__PublicUrl` — opcional, default el dominio `pub-….r2.dev` público del bucket. `R2__AllowedPhotoSourceHosts` — opcional, allowlist SSRF de fuentes de descarga (default: places.googleapis.com, \*.googleusercontent.com, \*.ggpht.com, wanderlog.com y subdominios).
- Con config activa, la ingesta (create/bulk/import-from-urls) rehostea cada foto server-side (descarga validada → webp máx 1200px vía ImageSharp → upload S3, cliente S3 con `Timeout=10s`/`MaxErrorRetry=1`) y `Place.Photos` solo persiste URLs de R2; una URL de Google con `key=` que falle el rehost se descarta (nunca se persiste). Un fallo/cuelgue de R2 degrada a "sin foto" — jamás aborta la creación del place. Circuit breaker de ingesta (`IngestPhotoBreaker`, por-request, compartido en bulk/import): abre por INTENTOS (3 fallos consecutivos de upload) **y por TIEMPO** (`R2__IngestRehostBudget`, default 25s, fracción del deadline del proxy de Railway) — acotar solo intentos no basta, un R2 colgado agota el proxy (~40s) con ~2 uploads antes del 3er fallo. Al abrir (por cualquiera de los dos) deja de intentar rehost en el resto de la request; `RehostForIngestAsync` acota cada upload al presupuesto restante (linked CTS) y captura el `OperationCanceledException` del rehost degradando a "sin foto" **con INDEPENDENCIA de la fuente del ct** — budget O caller (ronda 4: en ingesta un R2 colgado NUNCA es fatal; el guard `when(!ct.IsCancellationRequested)` se quitó porque un abort del proxy —`RequestAborted`, tras pre-work que agotó el deadline— propagaba y reventaba el bulk/import). El path de BACKFILL (`RehostAsync` directo) conserva su guard y sí propaga un cancel real. No re-descarga de Google ni propaga el fallo de R2 como fallo de creación. Import masivo commitea por chunks de 10 **con un CT que sobrevive al `RequestAborted`** (`CancellationToken.None`): la degradación incondicional + el commit que sobrevive al abort garantizan que todos los places se creen (sin foto), la request no reviente y nada del gasto ya incurrido se pierda (ronda 4).
- `POST /admin/places/backfill-photos` migra lo existente (excluye `deleted`/`rejected`) y devuelve censo por dominio. Un solo barrido a la vez (409 `backfill_already_running`); fallos por-fuente entran en backoff (`deferredPlaces`, reintento con `retryDeferred=true`); 3 fallos seguidos de upload a R2 abortan el barrido (`aborted`/`abortReason`) para no facturar Google sin progreso; `recoverMissing=true` recupera places sin fotos vía `GooglePlaceId` (Places Details). **Señal de parada del bucle = `converged:true`** (`!aborted && remainingPlaces==0 && failedPlaces==0 && deferredPlaces==0 && unmigratedPlaces==0`), NO `remainingPlaces==0` a secas: un fallo transitorio de fuente en el run difiere un place migrable sin subir `remainingPlaces`, y un fallo de UPLOAD sub-umbral (streak<3) deja el place sin migrar sin diferirlo → cuenta como `unmigratedPlaces` (ronda 3). `deferredPlaces` se recalcula al final del run. Runbook: `Shared/Photos/README.md`.
- **⚠️ Gate de deploy (bloqueante)**: el filtro `key=` a nivel DTO entra en vigor al desplegar; con prod aún lleno de URLs de Google la app quedaría sin imágenes. Orden obligatorio: (1) credenciales R2 en Railway → (2) backfill en bucle hasta `remainingPlaces=0` → (3) recién entonces deploy. Runbook completo y diseño: `Shared/Photos/README.md`.
- Concurrencia: `Place` usa `xmin` de Postgres como concurrency token (migración no-op `UseXminConcurrencyOnPlaces`). El backfill hace reload+merge ante conflicto; `PATCH /admin/places/:id` (y review/postpone) devuelven 409 `concurrent_update` en escritura stale — nunca lost update silencioso de `Photos`. Los demás escritores de `Place` (`DELETE`, `reindex-embeddings`, `backfill-descriptions`, `translate-batch`, `backfill-opening-hours`) también enrutan su `SaveChanges` por `TrySaveChangesAsync` → 409 graceful, no 500.
- Defensa a nivel DTO: `PlaceDto`/`ResolvedPlaceDto` **y `AdminPlaceDto`** filtran cualquier URL con `key=` (`PhotoUrls.Sanitize`). Las URLs con key solo viajan en `GooglePlacePreview` (google-search) como input efímero del import.
...
| `BackfillPhotosCoordinator` (lock + deferral del backfill de fotos) | Singleton in-process | El lock no serializa entre réplicas → barridos concurrentes duplican descargas de Google (doble facturación) y escrituras R2; el backoff de fallidos tampoco se comparte. Migrar a advisory lock de Postgres antes de escalar. |
...
| Admin — Places | `GET /admin/places/cities`, `POST /admin/places/google-search`, `GET /admin/places`, `GET /admin/places/:id`, `POST /admin/places`, `POST /admin/places/bulk`, `POST /admin/places/import-from-urls`, `PATCH /admin/places/:id`, `PATCH /admin/places/:id/review`, `PATCH /admin/places/:id/postpone`, `DELETE /admin/places/:id`, `POST /admin/places/reindex-embeddings`, `POST /admin/places/backfill-opening-hours`, `POST /admin/places/backfill-photos` (migra fotos a R2; idempotente y convergente — 409 si ya hay un barrido; params `limit`, `dryRun`, `recoverMissing`, `retryDeferred`; censo por dominio en la respuesta; runbook en `Shared/Photos/README.md`), `POST /admin/places/:id/translate`, `POST /admin/places/:id/suggest-description`, `POST /admin/places/backfill-descriptions`, `POST /admin/places/translate-batch` |
