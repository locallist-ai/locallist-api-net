...
- **Clonar un plan (`POST /plans/:id/clone`)**: "guardar este plan como mío" — gancho de conversión del onboarding (el showcase se muestra sin cuenta; guardarlo exige registro, endpoint `[Authorize]` AppScheme → anónimo 401). Origen CLONABLE = `IsShowcase==true` (curado admin, clonable incondicional) O `Visibility=='public'` (plan de usuario público, honra bloqueos vía `IPlanAccessService.CanView`); privado/unlisted ajeno o inexistente → 404 opaco. Crea copia PROFUNDA propiedad del caller: nuevo Guid, TODOS los `PlanStop` con nuevos ids (place_id/orden/día/horario copiados fielmente), `CreatedById=caller`, `Visibility='private'`, `Source="cloned"`, `IsShowcase=false`, `ClonedFrom={origen}`; i18n (`NameI18n`/`DescriptionI18n`) copiado tal cual. **`source="cloned"` = plan de usuario NORMAL**: NUNCA sale en showcase (`GET /plans?showcase=true` filtra por `IsShowcase`) ni se trata como curated (el gate i18n `isCurated` mira `Source=="curated"`, así que el ES del showcase se conserva sin exigir `translation_status` approved). Cuenta como plan guardado → mismo gate del cupo que `POST /plans` (free con `FreeSavedPlansLimit` → 403 `saved_plans_limit_reached`; Plus ilimitado; tier fresco de DB vía `TierGate`). **Idempotencia**: si el caller ya tiene un plan activo con ese `ClonedFrom` (DELETE es hard, "activo"=existe), se devuelve ese (200) — anti doble-tap, no duplica; clon nuevo → 201. `cloned_from` (S1) es plan→plan del clone; distinto de `imported_from_platform`/`imported_creator_handle` (F2, vídeo→plan).
